...
import helpuqrMasterCopy1_dt1day
import importlib
import pandas as pd
...
if __name__ == '__main__':                          
    ...
    for i in range(0,runs):
        ...
        def initPlant(simInit, condition):
            ...
            name =   sys.argv[1]#"P3_plant"  # "Triticum_aestivum_adapted_2021"#
            pl.readParameters( name + ".xml")
            assert name in ['P0_plant','P1_plant','P2_plant','P3_plant']
            if name == 'P0_plant':
        ...
        maxSim = 28
        #dt = 1/24 #in days
        dt =   float(sys.argv[2])#1/24   # 10min
        ...
        while simDuration < maxSim:
            ...
            rC4.maxLoop = 500
            try:
                leafleafbladeSurface_ = np.array(rC4.plant.leafBladeSurface)[np.array(rC4.plant.getSegmentIds(4))]
                idsC4 = np.where(np.array(leafleafbladeSurface_)> 0)[0]
            except:
            ...
            if len(idsC4)>0:
                seg_leaves_idx = np.array(rC4.plant.getSegmentIds(4)) #get the leaf seg ids
                node_leaves_idx = seg_leaves_idx + 1 #get the leaf node ids
                blades= np.array(rC4.plant.leafBladeSurface)[seg_leaves_idx] #get the leaf blade size only for leaf segments
                Anmean= np.mean(np.array(rC4.An)[idsC4]*1e6) #get the mean An [micromol C m-2 s-1]
                AnTot = np.array(rC4.An)* np.array(rC4.plant.leafBladeSurface)[seg_leaves_idx]*2*1e6 # tot An method 1 [micromol C s-1]
                AnTotBis = np.array(rC4.Ag4Phloem)[node_leaves_idx]*12/(24*60*60)*10000/1000*1e6# tot An method 2 [micromol C s-1]
                print("sums", sum(blades)*2, Anmean, sum(AnTot), sum(AnTotBis)) # check that tot An method 1 and tot An method 2 are the same
                if max(abs(AnTot - AnTotBis)) > 1e-10:
                    print(AnTot - AnTotBis)
                    raise Exception                                                                                                 
                idsLeafBlade = np.where(np.array(rC4.plant.leafBladeSurface) >0)[0]
                print('simduration',simDuration, max(0.,sinusoidal(simDuration) - 0.3))
                print('An:{}, Vj:{}, Vp:{},Vc:{}, Rd:{}, Rd_ref:{}'.format(
                        np.mean(np.array(rC4.An)[idsC4]),np.mean(np.array(rC4.Vj)[idsC4])*1e6,
                ...
                Antot.append(sum(AnTot))#  µmol CO2 s-1

                AnC4.append(np.array(rC4.An)[idsC4]*1e6)
                gc.append(np.mean(np.array(rC4.gco2)[idsC4]) )
                Ev.append(np.mean(np.array(rC4.Ev)[idsC4]) * 1e6)

                AnC4_mean.append(Anmean)
                ...
                AnC4_mean.append(np.nan)
                FW_mean.append(np.nan)
                VjC4.append(np.nan) 
                VpC4.append(np.nan) 
                VcC4.append(np.nan)
                RdC4.append(np.nan)
                Rd_refC4.append(np.nan) 
                ci.append(np.nan) 
                cics.append(np.nan) 
                
            # cs.append(np.mean(np.array(rC4.cs)[idsC4])*1e6) 
