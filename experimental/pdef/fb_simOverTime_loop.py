...
for i in range(0,runs):
    ...
    while simDuration < maxSim:
        ...
        idsLeafBlade = np.where(np.array(rC4.plant.leafBladeSurface) >0)
        ...
        # Antot.append(np.sum(np.array(rC4.An)[idsC4]*1e6*np.array(rC4.plant.leafBladeSurface)[idsLeafBlade]))
        ...
        FW_mean.append(np.mean(np.array(rC4.fw)[idsC4])*1e6)
    ...
    results = pd.DataFrame({'An':AnC4, 'gco2':gc,'An_mean':AnC4_mean, 'Fw_mean':FW_mean, 
