...
for i in range(0,runs):
    ...
    def initPlant(simDuration, condition):
        ...
        # simDuration = 28 # [day] init simtime
        # spellDuration = 5
    ...
    for i in range(len(variables)):
        ...
        for j in range(len(variables[i])):
            ...
            ana.write(str(name)+"_results"+directoryN+"photo_2_"+str(j)+".vtp", ["psi_Xyl"]) 
            ...
            print('Fw:',np.mean(rC4.fw), 'gco2:', np.mean(rC4.gco2), 'Ci/Cs:', np.mean(np.array(rC4.ci)[idsC4]/rC4.cs))
            ...
            An.append(np.sum(np.array(r.An)[idsC4]) * 1e6)
            AnC4.append(np.sum(np.array(rC4.An)[idsC4]) * 1e6)
            FW.append(np.mean(rC4.fw))
        resultsAnC3[i] = An
        resultsAnC4[i] = AnC4
        resultsFwC4[i] = FW
    ...
    resultsFwC4 = pd.DataFrame(resultsFwC4)

    variables = pd.DataFrame(variables)

    # resultsAnC3.to_csv("results/"+str(name)+"_resultsAnC3"+str(i)"+.csv", index=False)
    resultsAnC4.to_csv("results/"+str(name)+"_resultsAnC4_"+str(i)+".csv", index=False)
    resultsFwC4.to_csv("results/"+str(name)+"_resultsFwC4_"+str(i)+".csv", index=False)
...
# ana.write(str(name)+"_testphoto.vtp", ["AnC4","organType", "subType", "isInSoil"]) 
